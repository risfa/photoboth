...
$strsql = "SELECT vmenu, ilevel, ttampil, idparent, vbodyid, numakses, vurl, tposisi, ttampil,creby, UNIX_TIMESTAMP(cretime) as cretime, modby, UNIX_TIMESTAMP(modtime) as modtime FROM ms_admin_menu WHERE idmenu = ".$db->escape($id)."";
$rs = $db->get_row($strsql);
if(!$rs){header("Location: index.php");exit;};
...
<div class="wrap">
  ...
  <form name="frmadd" method="post" action="edit_.php" autocomplete="off" onsubmit="getisifimage();">
	...
	<?php
		...
		(isset($_SESSION["vmenu"])) ? $vmenu = $_SESSION["vmenu"] : $vmenu = $rs->vmenu;
		(isset($_SESSION["selparent"])) ? $selparent = $_SESSION["selparent"] : $selparent = $rs->idparent;
		(isset($_SESSION["selnumakses"])) ? $selnumakses = $_SESSION["selnumakses"] : $selnumakses = $rs->numakses;
		(isset($_SESSION["txtbodyid"])) ? $txtbodyid = $_SESSION["txtbodyid"] : $txtbodyid = $rs->vbodyid;
		(isset($_SESSION["txturl"])) ? $txturl = $_SESSION["txturl"] : $txturl = $rs->vurl;
		(isset($_SESSION["selposisi"])) ? $selposisi = $_SESSION["selposisi"] : $selposisi = $rs->tposisi;
		...
		if(isset($_SESSION["tampil"]) && $_SESSION["tampil"]!=""){
		...
		if(isset($_GET["level"]) && is_numeric($_GET["level"])){
			...
			if(isset($_SESSION["sellevel"]) && is_numeric($_SESSION["sellevel"])){
				$plevel = $_SESSION["sellevel"];
			}else{
				$plevel = $rs->ilevel;
			}
		}
		...
		for($i=1;$i<=100;$i++){
			$cekava = $db->get_row("SELECT vmenu FROM ms_admin_menu WHERE numakses=".$db->escape($i)." AND ilevel=1 AND idmenu!=".$db->escape($id)."");
			if($plevel==1){				
				if(!$cekava){
